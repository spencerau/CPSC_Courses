...
*/
template <typename T>
MStack<T>::MStack(int iSize, char o){ //O(1)
	top = -1;
	count = 0;
	max = iSize;
	stack = new T[max];
...
template <typename T>
int MStack<T>::size(){ //O(1)
  	return count;
}

template <typename T>
bool MStack<T>::isEmpty(){ //O(1)
  	return (count==0);
}

template <typename T>
bool MStack<T>::isFull(){ //O(1)
  	return (count==max);
}
...
*/
template <typename T>
void MStack<T>::push(T c){ //O(max) linear
...
template <typename T>
T MStack<T>::pop() { //O(1) better tell user to check if empty first
...
template <typename T>
T MStack<T>::peek() { //O(1) what about if empty?
	if (isEmpty()) return NULL;
  	return stack[top];
}
